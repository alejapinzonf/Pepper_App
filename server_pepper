...
def connect_pepper(ip="127.0.0.1", port=9559):
    try:
        session.connect("tcp://{}:{}".format(ip, port))
    except RuntimeError:
        print("Error connecting to Pepper")
        sys.exit(1)
    ...
    def do_POST(self):
        ...
        if path == "/say":
            ...
            if len(parts) == 4:
                category = parts[2]
                name = parts[3]
                if category in GESTURES:
                    if name in GESTURES[category]:
                        pepper["animation"].run(GESTURES[category][name])
